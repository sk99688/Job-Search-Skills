...
2. **Pay floor: $4,000 USD/month — equivalently $48,000/year.** Raised from $40k/yr on 2026-09-22 and restated monthly, because monthly-quoted pay is where the floor actually bites and where it was being missed.
   - **Read the pay off the listing page, not the list/category view.** This is the whole reason the rule kept failing: Fuku's listing plainly stated **$900–$1,000 per month** (~$11k/yr) while the row recorded "Not disclosed", because nobody opened it. An unread salary is not an absent salary.
   - Monthly figures below $4,000/month are a drop, whatever they annualise to. Convert other currencies first — ₹50,000/month is roughly $570 and fails badly, even though the number looks large.
   - Annualise **ongoing hourly** rates ("$65/hr × 40hr × 52wk ≈ $135k") — but **never annualise per-task, per-project or piece-rate gig pay.** Doing so manufactures a salary that does not exist: mercor was recorded as "$85/hour ≈ $176k/yr" when the listing said $400 per accepted task at ~15hr/week. Record gig pay verbatim and treat the floor as unestablished.
   - If a listing states **no pay at all** (common on Wellfound/YC), keep it — tag `Salary: not disclosed — verify ≥$4k/month before applying`. Do not fabricate a number.
   - `scripts/validate_rows.py` checks this mechanically, so a monthly figure under the floor fails before the row can be written.
...
5. **Company size cap: under 200 employees.** Drop any listing whose company has 200 or more people (Wellfound/LinkedIn bands like "201-500" and "500+", or a researched headcount at or above 200). The 51-200 band is **in** scope — this cap was relaxed from 50 to 200 on 2026-09-22, because ≤50 was excluding genuinely good small-to-mid companies like Toggl (~130) and LottieFiles (~117).
...
6. **No single-country restriction — and that now includes India itself.** Only three kinds of location clear this filter:
   - **Worldwide** — "remote, anywhere", "Anywhere in the World", or a list naming 10+ countries.
   - **Multi-country including India** — open to several countries or regions, one of which is India ("Georgia, India, Jersey, Mexico, United States"; "APAC, LATAM, North America, Oceania"). Tag `Remote Fit: Multi-country`. These are international roles the user is eligible for, which is the point.
   - **Genuinely unstated** — the listing exposes no location at all. Tag `Unclear — verify on listing`.

   Everything else is a drop: "Remote only, Canada", "Remote, Europe", "Remote only, Mexico City", "Remote, LatAm", "London +1", "Remote only, Australia +4" — **and, as of 2026-09-22, "Remote, India", "Remote only, India", "Bengaluru +4", "Onsite or remote, Mumbai +1" and every other India-scoped listing.**

   **Why India-scoped roles are now excluded**, since this reverses the earlier rule and a future reader will otherwise assume it's a mistake: the user is in India and is looking for *international* remote work, not the Indian domestic market. Domestic listings pay local rates that rarely approach the $4,000/month floor in rule 2 — the two rules that changed on the same day are the same decision viewed from different angles. Fuku at $900-1,000/month and a ₹50,000/month intern role are what this filter is actually excluding.

   The distinction that matters: **India-only is a drop, India-included is a keep.** A role open to five countries one of which is India is an international role, and dropping it would throw away exactly what the user wants. Removing India from the location string and asking "is anywhere else still listed?" is the test.
...
7. **Write to Airtable — never the Artifact tool, never the old local HTML file.** This board lives in Airtable now (migrated 2026-09-22). Fixed IDs, don't look these up each run:
   ...
   Field schema (all already created — never call `create_table` again for this base): `Title`, `Company` (single line text), `Category` (single select: Full Stack / AI Engineer / Applied AI / Gen AI / Data Engineer), `Match` (single select: Direct / Partial (~50%) — per rule 1), `Platform` (single select: Wellfound / Work at a Startup (YC) / Arc.dev / LinkedIn / Himalayas / We Work Remotely / Built In / Other — the Glassdoor, ZipRecruiter, Indeed and Turing options still exist in Airtable from earlier runs but are retired; never write them), `Size`, `Salary`, `Posted`, `Location Scope` (single line text), `Remote Fit` (single select: Worldwide / India / Unclear), `Level`, `Job URL` (url), `Company Website` (url), `Company LinkedIn` (url), `Company Notes` (long text), `Link Check` (single select: Live / Blocked - verify manually / Removed - verify), `Applied` (checkbox), `Status` (single select: New / Applied / Interviewing / Rejected / Offer), `Notes` (long text).

   **`Company Website`, `Company LinkedIn` and `Company Notes` are part of every row, not optional extras.**

   `Company LinkedIn` is the company's LinkedIn page (`linkedin.com/company/<slug>`) — never a personal profile, a job posting, or a search URL. It earns its place for two reasons beyond convenience: LinkedIn states employee count directly ("11-50 employees", "201-500 employees"), which is frequently the only way to settle rule 5 when the board hides size; and it shows whether the company is actually active and hiring, which a static careers page does not. When the "company" is a staffing agency or marketplace, record its page but note that the headcount shown is the agency's, not the employer's, so it does not settle the cap for that row.
 Most job boards show a company name and nothing else, which leaves the user unable to judge whether a role is worth an application without searching the company themselves for every single row. Fill `Company Website` with the company's own domain — its actual product site, never a Wellfound/LinkedIn/Crunchbase profile — and `Company Notes` with one line on what they do plus any team-size or funding-stage signal you found. The size signal matters especially, since rule 5 caps at 200 employees and most boards don't disclose headcount, so a note like "Series B, ~130 people" is often the only way that rule can be applied at all — this research is exactly what caught NewRocket at ~490 and Sticker Mule at several hundred, both of which showed no size on their listing. Leave either link blank rather than guessing — a wrong domain, or the LinkedIn page of a different company with a similar name, is worse than an empty field, because the user will click it and act on what they find. Several companies in this space are small Indian consultancies with near-identical names, so this is a live risk, not a hypothetical one. When the "company" is a staffing agency or talent marketplace (Lemon.io, A.Team, Toptal, Proxify, Arc Exclusive and similar), say so in `Company Notes`: the end client is unknown, so neither the size rule nor the company research means what it normally would.
